...
@section('content')
    ...
    <form method="POST" action="{{ url('/update-hardware/'.$hardware->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                            ...
                            <div class="card-body">
                                ...
                                <div id="MainContent_divForm" class="row">
                                    <div class="col-12" style="overflow: auto; max-height: 500px;">
                                            ...
                                            <div class="col-3">
                                                <b>
                                                    ...
                                                    <select name="Company" class="form-control-sm text-bold text-xs">
                                                        <option value="ASMI" {{ $hardware->company == 'ASMI' ? 'selected' : '' }}>ASMI</option>
                                                        <option value="TIBSI" {{ $hardware->company == 'TIBSI' ? 'selected' : '' }}>TIBSI</option>
                                                        <option value="CPN" {{ $hardware->company == 'CPN' ? 'selected' : '' }}>CPN</option>
                                                        <option value="PMCC" {{ $hardware->company == 'PMCC' ? 'selected' : '' }}>PMCC</option>
                                                        <option value="ACSFI" {{ $hardware->company == 'ACSFI' ? 'selected' : '' }}>ACSFI</option>
                                                    </select>
                                                </b>
                                            </div>
                                            <div class="col-3">
                                                <b>
                                                    ...
                                                    <select name="Status" class="form-control-sm text-bold text-xs">
                                                        <option value="SPARE" {{ $hardware->status == 'SPARE' ? 'selected' : '' }}>SPARE</option>
                                                        <option value="USED" {{ $hardware->status == 'USED' ? 'selected' : '' }}>USED</option>
                                        ...
                                        <div class="row" style="margin-bottom: 20px;">
                                            <div class="col-3">
                                                <label  class="text-xs">Asset Name</label>
                                                <input name="Asset" type="text"  value="{{ old('Item', $hardware->asset_name) }}" class="form-control form-control-border form-control-sm text-xs" placeholder="Enter Item Name">
                                            </div>
                                            <div class="col-3">
                                                <label class="text-xs">Model No. / Name</label>
                                                <input name="Model" type="text"  value="{{ old('Model', $hardware->model) }}" class="form-control form-control-border form-control-sm text-xs" placeholder="Enter Model No. / Name">
                                            </div>
                                            <div class="col-3">
                                                <label class="text-xs">Manufacturer</label>
                                                <input name="Manufacturer" type="text"  value="{{ old('Manufacturer', $hardware->manufacturer) }}" class="form-control form-control-border form-control-sm text-xs" placeholder="Enter Manufacturer">
                                            </div>
                                            <div class="col-3">
                                                <label  class="text-xs">Serial / Code No.</label>
                                                <input name="Serial_no" type="text"  value="{{ old('Serial_no', $hardware->serial_number) }}" class="form-control form-control-border form-control-sm text-xs" placeholder="Enter Serial / Code No.">
                                            </div>
                                            <div class="col-3">
                                                <label  class="text-xs">Unit</label>
                                                <input name="Unit" type="text"  value="{{ old('Unit', $hardware->unit) }}" class="form-control form-control-border form-control-sm text-xs" placeholder="Enter Unit">
                                            </div>
                                            <div class="col-3">
                                                <label class="text-xs">Date</label>
                                                <input name="Date" type="date"  value="{{ old('Date', $hardware->date) }}" class="form-control form-control-border form-control-sm text-xs">
                                            </div>
                                            <div class="col-3">
                                                ...
                                                @if ($hardware->image)
                                                    <div class="mt-2">
                                                        <img src="{{ asset('uploads/hardware/'.$hardware->image) }}" width="50px" height="50px" alt="Image">
                                                        <span class="ml-2">{{ $hardware->image }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="col-3">
                                                <label class="text-xs">Rermarks</label>
                                                <input name="Remarks" type="text"  value="{{ old('Remarks', $hardware->remarks) }}" class="form-control form-control-border form-control-sm text-xs">
                                            </div>
                                            <div class="col-3">
                                                <label class="text-xs">Assigned To</label>
                                                <input name="Assigned" type="text"  value="{{ old('Assigned', $hardware->assigned_to) }}" class="form-control form-control-border form-control-sm text-xs">
                                            </div>
                                    ...
                                    <div class="card-footer float-right">
                                        ...
                                        <div class="pull-right">
                                            <button type="submit" id="saveCom" class="btn btn-success btn-sm btn-sm"><i class="fa fa-arrow-alt-circle-right"></i>Save</button>
                                                <a id="CancelCom" class="btn btn-danger btn-sm btn-sm" href="{{ url('/hardware') }}"><i class="fa fa-times-circle"></i>Cancel</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
